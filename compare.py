...
old = pd.read_csv('MyRecord.csv', na_values=['NA'])
new = pd.read_csv('Feb-4pm.csv', na_values=['NA'])
old['version'] = "old"
new['version'] = "new"
...
changes = full_set.drop_duplicates(subset=["NAME"],keep='last')

#We want to know where the duplicate NAMES are, that means there have been changes
